...
#    precipitation sensor. It is a tipping bucket.
#
...
#       better???
#    4. Separate major sections into callable functions: reader, processor,
#       output, plotter, etc.
#    5. Add ability to plot using different units (e.g. inches)
#
#
#
#How to Use:
...
#           wildcard
#           tag
#    2. Run with "python RAIN-GAUGE_reader-plotter.py" in terminal, or open in
...
#
#       e.g. plt.savefig('%s%s_RAIN_%s.png' % (save_dir, site_ID, tag), dpi=500, \
#                        bbox_inches='tight')
#
...
#       is requested for plotting, which is a bit of an overkill; the only way
#       to read in subsets of time is by using the wildcard option to specify
#       certain files, but this, too, is limited.
...
directory = "/Users/blund/Documents/3D-PAWS/Data/Kenya_GLOBE_1/wx_stn/rain/"
...
save_dir = "/Users/blund/Documents/Python_Scripts/Figures/CSA_3DPAWS01/RAIN/"

#set this variable to "mm" to plot in millimeters, or "inches" to plot in
#    inches
units = "mm"
...
if units == "mm" or units == "inches":
    ...
    print("'mm' for millimeters")
    print("'inches' for inches")
    sys.exit()
...
rain = [] #visible light in Watts per meter squared
...
if num_data_gaps > 0: #NaNs are not included in 'num_data_gaps'
    ...
    #    of uptime/downtime
    missing_reports = df.rain.isna().sum()
...
rain = np.array(df.rain, dtype=float)
...
missing_reports = df.rain[mintime:maxtime].isna().sum()
...
#plot data based on the user-selected units; plot parameters within these
#    'if' statements are those specific to each variable
#plot
ax = df.plot(x='time', y=['rain', 'no_rain'], color=['b','r'], label=['rain','no-rain'], figsize=(30,5))
...
plt.title("%s : Tipping Bucket" % site_ID, fontsize=14)
...
# #save the figure with a name based on the user-selected units
# if units == "mm":
#     plt.savefig('%s%s_RAIN-%s_%s.png' % (save_dir, site_ID, units, tag), \
