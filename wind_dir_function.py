...
# -*- coding: utf-8 -*-
"""
Created on Fri Jul 24 14:11:33 2020
...
#    wind vane sensor. It should be used in combination with another parent
...
#       extensions):
#
#       a) from wind_dir_function import wind_dir_processor
#       ... or...
#       b) import wind_dir_function as wdf
#
#    3. Call the function in the parent program, ensuring that you pass the 
#       appropriate attributes/parameters:
#
#       a) call_winddir = wind_dir_processor(directory, wildcard)
#       ... or...
#       b) call_winddir = wdf.wind_dir_processor(directory, wildcard)
#
#    4. Run the parent program with in terminal (e.g. "python WINDDIR_reader-plotter.py"),
...
def wind_dir_processor(directory, wildcard):
    #tell the user the function was called
    print("------------------------------------------------------------------\n")
    print("'wind_dir_processor' function called...\n")
    ...
    time = []
    for i in range(len(month)): #the variable could be any of the time
        #    variables (i.e. 'month', 'day', 'year', 'hour', 'minute') since
        #    each should be the same length. This is to include every time
        #    from every file.
    ...
    #    NOTE: time is not reported to the second in these files, but the line
    #    the below will output the date-time elements in HH:MM:SS format so
    #    the assumption is that records are reported at the top of the minute,
    #    though not explicitly true. (Can I change the output format?)
    ...
    ''' This section has no bearing on plotting and data cleansing. It is
        intended for catalogging and analysis of raw data purposes only. '''
    ...
    #indices of times in the dataFrame where the difference between i+1 and i
    #    is negative (i.e. out of chronological order)
    ...
    #the number of occurrences such that timestamps are out of chronological
    #    order
    ...
    #sort rows chronologically by 'time' by setting 'time' as the index
    #    variable
    ...
    #    16:30 rounds to 16:00); we need every occurrence of a half-minute to
    #    round up. The function(s) below does just that.
    ...
    if pd.Index(df.time).has_duplicates == True:
        ...
        #keep a list of each duplicated timestamp; the method below will give
        #    ALL instances of a duplicated timestamp, so if there are 3
        #    occurrences for the same time, two will appear in the list; by
        #    default the first occurrence of a duplicated timestamp is False
        ...
        #convert the list of duplicated times to strings to later be output to
        #    a text file
        ...
        #remove any and all rows that have the same timestamp (i.e. where
        #    there are duplicated times), but preserve the first occurrence;
        #    the following line first resets the index variable such that
        #    'time' is a regular column like the rest of the data, then drops
        #    rows based solely on duplicates in 'time' (i.e. subset='time'),
        #    but keeps the first occurrence of any duplications (i.e.
        #    keep='first'), then sets the index back to the 'time' column
        #    (this will be necessary later on)
        df = df.drop_duplicates(subset='time', keep='first')
        #NOTE: the above method assumes the first occurrence of a duplicated
        #      timestamp contains the correct/valid data, which is not
        #      necessarily true. To be handled later...
        ...
        #    such that no data were read in, 'time' in the data frame (or any
        #    other variable for that matter) should be empty. If so, raise an
        #    error so the program exits
    ...
    #Now we check to see if there are any missing report times. Timestamps
    #    should be monotonically increasing by 1 minute intervals. Since the
    #    interval is not exactly 1 minute (sometimes it is 61 seconds), the
    #    total number of data gaps is not perfectly representative of the
    #    exact number of data gaps. Due to the sublte inconsistencies in time
    #    reporting intervals and the time rounding method, instances such as...
    #    2:55:29, 2:56:29, 2:57:30 rounding to 2:55:00, 2:56:00, 2:58:00...
    #    will include 2:57 as a data gap when in fact, this is not necessarily
    #    true
    num_data_gaps = (df.time.diff() > pd.Timedelta(minutes=1)).sum()
    #NOTE: the value contained in 'num_data_gaps' is not necessarily
    #      representative of the number of missing reports because each data
    #      gap could contain multiple missing reports (e.g. a time gap from
    #      14:10 UTC to 14:15 UTC counts as only 1 data gap but accounts for 4
    #      missing minutely reports {14:11, 14:12, 14:13, and 14:14 UTC})
    
    if num_data_gaps > 0: #NaNs are not included in 'num_data_gaps'
        print("There are %s data gaps. Filling data gaps with NaNs...\n" % num_data_gaps)
        
        # fill in the gaps #
        #create a DatetimeIndex ranging from the oldest time in the 'time'
        #    column to the newest time using an interval of 1 minute
        time_full = pd.date_range(start=df.time.min(), end=df.time.max(),freq='min')
        
        #create a DatetimeIndex using the full list of times above to serve as
        #    the new time array and index
        new_index = pd.Index(time_full, name="time")
        
        #set the index as 'time' for reindexing below
        df = df.set_index('time')
        
        #re-index the DataFrame using the new list of times, then reset the
        #    index. This will replace the original, incomplete list of
        #    timestamps while simultaneously filling in the elements for all
        #    other variables as NaNs where there were previously no records
        df = df.reindex(new_index).reset_index()
        
        #now, count the column (any column) sum of NaNs; THIS will tell you
        #    explicitly how many reports are missing and a better
        #    representation of uptime/downtime
