...
#
#How to Use:
#    1. Save a copy of the wind_spd_function files in the same directory as
#       this program
...
#           avg_window: an integer greater than zero
#           save_dir
#           wildcard
#           tag
...
#
#
#NOTES: the y-axis range may be site-dependent. These are currently hard-coded
#       in the plotting section for each variable. If you see no data with the
#       first attempt to plot a variable, try commenting out the following
#       line for the appropriate varaible:
#
...
directory = "/Users/blund/Documents/3D-PAWS/Data/WMO_HIGHWAY_14/data/wind_speed/"
...
elif len(mintime) == 16 and len(mintime) == 16: #they should include 19 characters
        try:
            pd.to_datetime(mintime, format='%Y-%m-%d %H:%M')
            pd.to_datetime(maxtime, format='%Y-%m-%d %H:%M')
        except ValueError:
...
missing_reports = df.wind_speed[mintime:maxtime].isna().sum()
